...
class AimlHandler(ContentHandler):
    ...
    def __init__(self, encoding=None):
        ...
        self._currentUnknown = ""  # 当前未知元素的名称

        # 当类别中出现解析错误时，此设置为true。
        self._skipCurrentCategory = False

        # 计算特定AIML文档中解析错误的数量。
        # 用getNumErrors()查询。如果为0，则文档是符合aiml格式的。
    ...
    def _validateElemStart(self, name, attr, version):
        ...
        try:
            ...
            # happen.
            raise AimlParserError(
                ("Element stack is empty while validating <%s> " %
                 name) + self._location())
        required, optional, canBeParent = self._validInfo[parent]
        nonBlockStyleCondition = (
            parent == "condition" and not (
        ...
        if not canBeParent:
            raise AimlParserError(
                ("<%s> elements cannot have any contents " %
                 parent) + self._location())
        ...
        # contain <li> subelements.
            ...
            raise AimlParserError(
                ("<%s> elements can only contain <li> subelements " %
                 parent) + self._location())
            ...
            if nonBlockStyleCondition:
                if "name" in parentAttr:
                    ...
                    if len(attr) == 0:
                        ...
                        raise AimlParserError(
                            "Invalid <li> inside single-predicate <condition> " + self._location())
                    ...
                    if len(attr) == 0:
                        ...
                        raise AimlParserError(
                            "Invalid <li> inside multi-predicate <condition> " + self._location())
        # All is well!
        return True
